...
# SMcElmurry, 2020Mar21,   Added Track class code, modified CD class methods to better handle tracks (see TODone)
#------------------------------------------#
...
class Track():
    ...
    def __init__(self, position: int, title: str, length: str) -> None:
        """Set ID, Title and Artist of a new Track Object"""
        #    -- Attributes  -- #
        try:
    ...
    def position(self, value):
        try:
            ...
            raise Exception('Track number needs to be an Integer')
    ...
    def __str__(self):
        ...
        return '{:>2}\t{} (Length {})'.format(self.position, self.title, self.length)
...
class CD:
    ...
    def cd_tracks(self,):
        try:
            ...
            raise Exception ("Object must be a list. ", str(e))
    ...
    def add_track(self, track: Track) -> None:
        ...
        # TODone sort tracks
        self.__sort_tracks()
        if len(self.__tracks) < track.position:
            self.__tracks.append(track)
        else:
            if self.__tracks[track.position - 1] == None:
                self.__tracks.append(track)
            else:
                raise Exception ("Track position already exists. ")
        self.__sort_tracks()
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        # TODone remove track
        # TODone sort tracks
        if len(self.__tracks) < track_id:
            print(track_id, " does not exist. Track listing has ID's up to: ", len(self.__tracks))
        else:
