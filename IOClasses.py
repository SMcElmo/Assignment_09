...
# SMcElmurry, 2020Mar21,   Modify FileIO class to accept list parameter (see TODone), added value_Errors to ScreenIO class
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            ...
            with open(file_name[1], "w") as file:
                for disc in lst_Inventory:
                    trackList = disc.cd_tracks
                    for tracks in trackList:
                        file.write(disc.get_record() + "," + tracks.get_record()+"\n")
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name[1], "r") as file:
                for line in file:
                    data = line.strip().split(",")
                    cdObj = PC.DataProcessor.select_cd(lst_Inventory, int(data[0]))
                    trackObj = DC.Track(data[3], data[4], data[5])
                    cdObj.add_track(trackObj)
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
...
class ScreenIO:
    ...
    def get_track_info():
        ...
        """
        trkId = ScreenIO.value_Errors(int,"Enter Position on CD / Album: ", "Position must be an integer!")
